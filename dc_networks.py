...
class MLP(nn.Module):
    def __init__(self, channel, num_classes):
        ...
        self.fc_1 = nn.Linear(28*28 if channel==1 else 32*32*3, 128)
...
class BSReduction(nn.Module):
    def __init__(self, input_bs, output_bs, channel, feat_size):
        super(BSReduction, self).__init__()

        channel_factor = [input_bs, 8, 1]
        module_list = []
        for idx in range(len(channel_factor) - 1):
            factor, next_factor = channel_factor[idx], channel_factor[idx+1]
            module_list.append(nn.Conv2d(channel * factor, channel * next_factor, kernel_size=3, padding=1))
            shape_feat = [channel * next_factor, feat_size[0], feat_size[1]]
            module_list.append(self._get_normlayer('instancenorm', shape_feat))
            module_list.append(self._get_activation('relu'))
...
class BSReduction_v2(nn.Module):
    def __init__(self, input_bs, output_bs, channel, feat_size):
        super(BSReduction_v2, self).__init__()

        module_list = []

        channel_factor = [input_bs, output_bs]  # Simplified directly to input_bs and output_bs
        for idx in range(len(channel_factor) - 1):
            factor, next_factor = channel_factor[idx], channel_factor[idx+1]
            module_list.append(nn.Conv2d(factor, next_factor, kernel_size=1))  # Removed padding since kernel_size=1 does not need it
